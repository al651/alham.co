...
function Blog() {
    return (
        ...
        {
            blogPosts.map((post) => 
            <BlogPost title={post.title} description={post.description} img={post.img} slug={post.slug} alt={post.imgalt} date={post.date} />
            )
        }
        </div>
    );
